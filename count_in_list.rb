...
def count_in_lists(list,item_to_count)
  counter = 0
  for i in list do
    print "i   - ",i,"   -   item_to_count   -   ",item_to_count,"   - list[i]   -  ",list[i]
    puts ""
    if list[i]==item_to_count
    counter += 1
    end
  end
  puts counter
...
if __FILE__ == $PROGRAM_NAME
  ...
  p count_in_list([1,2,3,4,5] , 3) == 1
  p count_in_list([1,2,1,2,1] , 2) == 2
  p count_in_list([0,2,3,4,5] , 1) == 0
  p count_in_list([0,0,2,1,0] , 0) == 3
end